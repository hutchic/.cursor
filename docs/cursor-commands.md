...
The [deslop command](.cursor/commands/deslop.md) is a code-cleanup workflow: compare the branch to main and remove AI-generated slop (extra comments, unnecessary defensive checks, `any` casts, style inconsistencies), then report a 1–3 sentence summary. The [land-the-plane command](.cursor/commands/land-the-plane.md) runs the full wrap-up workflow: sync taskmaster/beads (if in use), ensure quality gates pass, run deslop, then run shipit.
